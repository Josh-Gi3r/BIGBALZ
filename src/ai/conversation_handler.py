    ...
    async def get_response(self, message: str, user_id: int, 
        ...
        try:
            ...
            if self._is_insult_or_troll(message):
                ...
                # Add context for AI about being insulted with memory awareness
                memory_context = self.memory_manager.get_response_variation_context(user_id)
                
                troll_context = "Someone is trying to insult or troll you. Time to get EXTREMELY sassy. Give them a short, intelligent, savage comeback that shows your personality. Be witty and creative, not defensive or long-winded. Make it sting but keep it brief."
                
                if memory_context['interaction_count'] > 2:
                    troll_context += " You've interacted with this user before, so vary your savage responses - don't use the exact same insults or comebacks."
                ...
                if response:
                    self._update_history(user_id, message, response)
                    self.memory_manager.record_phrase_usage(user_id, "savage comeback", "troll_responses")
                    self.memory_manager.record_response_pattern(user_id, "savage")
                    return response
